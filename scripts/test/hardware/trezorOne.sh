...
# trezorOne.sh - Script for verifying Trezor One firmware reproducibility
# Usage: ./trezorOne.sh <version>
# Example: ./trezorOne.sh 1.13.0
...
version=$1
echo -e "${BLUE}Analyzing Trezor One firmware version ${version}${NC}"
...
# Download firmware files with correct URLs - using new t1b1 path
echo -e "${BLUE}Downloading firmware files...${NC}"
wget -q "https://data.trezor.io/firmware/t1b1/trezor-t1b1-${version}.bin" -O trezor-firmware.bin
wget -q "https://data.trezor.io/firmware/t1b1/trezor-t1b1-${version}-bitcoinonly.bin" -O trezor-firmware-bitcoinonly.bin
...
# Use the correct build command for Trezor One firmware
./build-docker.sh --models T1B1 --skip-core legacy/v${version}

# Check if build was successful by looking for the built firmware files
# Updated paths based on build output
built_standard_path="build/legacy-T1B1/firmware/firmware.bin"
built_bitcoin_path="build/legacy-T1B1-bitcoinonly/firmware/firmware.bin"
...
# Note the different signature offset for Trezor One (544 instead of 5567)
cp trezor-firmware.bin trezor-firmware.bin.zeroed
dd if=/dev/zero of=trezor-firmware.bin.zeroed bs=1 seek=544 count=195 conv=notrunc status=none

cp trezor-firmware-bitcoinonly.bin trezor-firmware-bitcoinonly.bin.zeroed
dd if=/dev/zero of=trezor-firmware-bitcoinonly.bin.zeroed bs=1 seek=544 count=195 conv=notrunc status=none
...
# Updated to use the correct paths based on build output
if [ -f "trezor-firmware/$built_standard_path" ]; then
    built_standard_hash=$(sha256sum "trezor-firmware/$built_standard_path" | cut -d' ' -f1)
...
if [ -f "trezor-firmware/$built_bitcoin_path" ]; then
    built_bitcoin_hash=$(sha256sum "trezor-firmware/$built_bitcoin_path" | cut -d' ' -f1)
